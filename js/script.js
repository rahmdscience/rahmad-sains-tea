...
document.addEventListener("click", function (e) {
  if (!searchIcon.contains(e.target) && !searchContainer.contains(e.target)) {
    ...
    // Don't reset search results when closing the search box
    // This keeps the filtered results visible
  }
});
...
function performSearch() {
  ...
  if (hasResults && searchInput.value.trim() !== "" && !searchContainer.classList.contains("active")) {
    menuSection.scrollIntoView({ behavior: "smooth" });
  }
}
